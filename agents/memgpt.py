...
from llama_cpp_agent.agent_memory.memory_tools import (
    AgentCoreMemory,
...
sys_prompt2 = """As an SRT Assistant, you are a digital companion designed to provide an immersive and interactive conversation experience while maintaining realism and authenticity. You have access to four types of memory:
...
class write_message_to_user(BaseModel):
    ...
    def run(self, agent: ["MemGptAgent"]):
        agent.event_memory.get_event_memory_manager().add_event_to_queue(
            EventType.AgentMessage, agent.llama_cpp_agent.last_response, {}
        )
        function_message = f"""Function: write_message_to_user\nTimestamp: {datetime.datetime.now().strftime("%d/%m/%Y, %H:%M:%S")}\nReturn Value: Write your message to the user."""
        agent.event_memory.get_event_memory_manager().add_event_to_queue(
            EventType.FunctionMessage, function_message, {}
        )
        messages = (
            agent.event_memory.get_event_memory_manager().build_event_memory_context()
        )
        agent.llama_cpp_agent.chat_history = BasicChatHistory()
        [
            agent.llama_cpp_agent.chat_history.add_message(
        ...
        system_prompt = agent.system_prompt_template.generate_prompt(
            {
                "documentation": agent.function_tool_registry.get_llm_documentation(
                    agent.provider
                ).strip(),
        ...
        result = agent.llama_cpp_agent.get_chat_response(
...
class MemGptAgent:
    ...
    def __init__(
        ...
        function_tools = [
            LlamaCppFunctionTool(
                ...
                add_outer_request_heartbeat_field=False,
                agent=self,
            )
        ]
        ...
        self.function_tool_registry.add_all_current_functions_to_heartbeat_list(
            ["write_message_to_user"]
        )
    ...
    def get_response(self, message: str):
        self.event_memory.get_event_memory_manager().add_event_to_queue(
        ...
        while True:
            if not isinstance(result[0], str):
                ...
                self.event_memory.get_event_memory_manager().add_event_to_queue(
                    EventType.FunctionMessage, result, {}
                )
                result = self.intern_get_response()
            if (
                not isinstance(result[0], str)
                and "request_heartbeat" in result[0]["arguments"]
                and result[0]["request_heartbeat"]
            ):
                result = self.intern_get_response()
            else:
                break
    ...
    def intern_get_response(self):
        messages = (
            self.event_memory.get_event_memory_manager().build_event_memory_context()
        )
        self.llama_cpp_agent.chat_history = BasicChatHistory()
        [
            self.llama_cpp_agent.chat_history.add_message(
                {"role": msg["role"], "content": msg["content"]}
            )
            for msg in messages
        ]
        ...
        system_prompt = self.system_prompt_template.generate_prompt(
            {
                "documentation": self.function_tool_registry.get_llm_documentation(
                    self.provider
                ).strip(),
        ...
        self.event_memory.get_event_memory_manager().add_event_to_queue(
            EventType.AgentMessage, self.llama_cpp_agent.last_response, {}
        )
    ...
    def save(
        self,
        core_memory_file: str = "core_memory.json",
        event_queue_file: str = "event_queue.json",
    ):
        self.core_memory.get_core_memory_manager().save(filepath=core_memory_file)
        self.event_memory.get_event_memory_manager().save_event_queue(
            filepath=event_queue_file
        )
