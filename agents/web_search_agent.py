...
async def web_search_response(message, history, system_message, max_tokens, temperature, top_p, top_k, repetition_penalty, model):
    try:
        ...
        if "llama_cpp_server" in default_agent_provider:
            ...
            yield f"Unsupported llama-cpp-agent provider: {default_agent_provider}"
            return
